...
function containsStrongCatchLanguage(text) {
  return /\b(
    caught|
    landed|
    bass (?:was|were) caught|
    bass (?:was|were) landed|
    caught a bass|
    caught bass|
    landed a bass|
    landed bass|
...
function getLocalEvidence(paragraph, markName) {
  ...
  for (const sentence of sentences) {
    if (
      aliases.some(alias =>
        new RegExp(`\\b${escapeRegex(alias)}\\b`, "i").test(sentence)
      )
    ) {
...
function getEvidenceType(text, markCount) {
  const bass = containsBass(text);

  if (!bass) {
    return "uncertain";
  }

  if (markCount === 1 && containsStrongCatchLanguage(text)) {
    return "confirmed_catch";
  }

  if (markCount === 1 && containsCatchLanguage(text)) {
...
function extractFishCount(text) {
  ...
  const patterns = [
    ...
    /\b(?:two|three|four|five|six)\s+(?:bass|sea bass)\b/i
  ];
...
function extractLength(text) {
  ...
  for (const regex of patterns) {
    ...
    if (match) {
      ...
      if (Number.isFinite(value) && value > 10 && value < 150) {
        return value;
      }
    }
  }
...
/*
  We intentionally do NOT attempt to guess lure names from
  ordinary article text.

  A future version can add lure extraction using a controlled
  lure vocabulary once enough real examples have been collected.
*/
...
function extractMethod(text) {
  if (/\blure|luring|lures\b/i.test(text)) {
    return "Lure";
  }

  if (/\bplug|hard lure|hardbait|minnow|surface lure|pencil\b/i.test(text)) {
    return "Lure";
  }

  if (/\bspinning|spinning rod|spinner\b/i.test(text)) {
    return "Lure";
  }

  if (/\bsurfcast|surf casting|bait\b/i.test(text)) {
    return "Bait";
  }
...
function buildReports(html, source) {
  ...
  let currentSection = "Sea fishing catch reports";

  for (const paragraph of paragraphs) {
    /*
      Heading-like paragraphs become section names.
    */
    if (
    ...
    /*
      No named mark:
      keep only as a regional report if it contains
      meaningful bass information.
    */
    if (marks.length === 0) {
      ...
      const evidenceType = "regional_report";
      ...
      reports.push({
        ...
        time: null,
        species: "Bass",
        fishCount: extractFishCount(paragraph),
        fishWeightLb: extractWeight(paragraph),
        fishLengthCm: extractLength(paragraph),
        method: extractMethod(paragraph),
        lure: null,
        notes: paragraph,
    ...
    /*
      A paragraph can mention more than one mark.
      We do NOT turn that into multiple confirmed catches.
      Instead each mark gets its own local evidence.
    */
    for (const mark of marks) {
      const localSentences = getLocalEvidence(
        paragraph,
        mark.markName
      );
      ...
      const evidenceType = getEvidenceType(
        localText,
        marks.length
      );

      /*
        If several marks occur in the same paragraph,
        we refuse to call it a confirmed catch.
      */
      const safeEvidenceType =
        marks.length > 1 && evidenceType === "confirmed_catch"
          ? "activity_mention"
          : evidenceType;

      const fishWeightLb =
        safeEvidenceType === "confirmed_catch"
      ...
      const confidence = calculateConfidence({
        ...
        localEvidence: localSentences.length > 0,
        fishWeightLb,
        fishLengthCm,
        fishCount
      });
      ...
      reports.push({
        id: stableHash(
          `${source.url}|${currentSection}|${mark.markName}|${safeEvidenceType}|${localText}`
        ),
        ...
        date: extractDate(localText) || extractDate(paragraph),
...
function sortReports(reports) {
  return reports.sort((a, b) => {
    ...
    return String(a.markName || "").localeCompare(
      String(b.markName || "")
    );
  });
}
...
async function main() {
  console.log("Bass Finder Wales catch collector starting...");
  ...
  const reports = sortReports(
    deduplicateReports(allReports)
  );

  const confirmed = reports.filter(
    report => report.evidenceType === "confirmed_catch"
  ).length;

  const activity = reports.filter(
    report => report.evidenceType === "activity_mention"
  ).length;

  const regional = reports.filter(
    report => report.evidenceType === "regional_report"
  ).length;

  console.log("");
  console.log("Collection complete.");
  console.log(`Total reports: ${reports.length}`);
  console.log(`Confirmed catches: ${confirmed}`);
  console.log(`Activity mentions: ${activity}`);
  console.log(`Regional reports: ${regional}`);
  console.log("");

  const existing = loadExisting();

  const oldComparable = existing
    ? comparableReports(existing.reports || [])
    : [];

  const newComparable = comparableReports(reports);
  ...
  const output = {
    ...
    confirmedCatchCount: confirmed,

    activityMentionCount: activity,

    regionalReportCount: regional,
